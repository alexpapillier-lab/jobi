-- Provize: zakázky vyloučené kvůli opravám jsou vidět a jdou započítat
-- ============================================================================
--
-- PROČ: Zakázka, na které zbyly jen vyloučené opravy (výměna těla, baterie…),
-- se do provizí dosud nezapsala vůbec – majitel neměl jak zjistit, co se
-- vyloučilo, ani to v odůvodněném případě započítat. Teď se zapíše jako
-- vyřazená s nulou a plnou cenou vedle; tlačítkem „Započítat i vyloučené
-- opravy“ se u ní počítá plná cena (pocitat_vse) a synchronizace to respektuje.

alter table public.provize_polozky add column if not exists zaklad_plny numeric(12,2);
alter table public.provize_polozky add column if not exists pocitat_vse boolean not null default false;

comment on column public.provize_polozky.zaklad_plny is
  'Cena zakázky včetně vyloučených oprav (jen u první položky). Vedle základu ukazuje, co se vyloučilo.';
comment on column public.provize_polozky.pocitat_vse is
  'Majitel u zakázky započítal i vyloučené opravy – základ je plná cena a synchronizace ho nesnižuje.';
...
-- Ruční přepnutí: počítat u zakázky plnou cenu / zase jen nevyloučené opravy.
...
begin
  ...
  select * into n from public.provize_nastaveni where service_id = p.service_id;

  update public.provize_polozky
  set pocitat_vse = p_zapnout,
      poznamka = case when p_zapnout then 'započítáno včetně vyloučených oprav' else null end,
      vyrazeno = case when p_zapnout then false else vyrazeno end
  where id = p_id;

  -- Základ srovná synchronizace (u zapnutí plná cena, u vypnutí zpět jen nevyloučené).
