-- KPI techniků: „Dokončil“ počítá zakázky, ne přechody, a nepočítá storno.
--
-- Revize 6. 9.: každý přechod do koncového stavu se počítal jako dokončení –
-- i storno (Zrušeno) a opakované uzavření téže zakázky. Teď se počítá každá
-- zakázka jednou a stavy, které jsou storno (klíč cancelled/storno nebo
-- název Zrušeno, Storno, Neopraveno, Odmítnuto – stejná heuristika jako
-- dialog při stornu v aplikaci), se vynechají. Běžící stopky se ořežou
-- koncem období, ať zapomenutý časovač nenafukuje jiné měsíce.
