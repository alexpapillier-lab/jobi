-- Allow ticket updates from service_role (auth.uid() IS NULL) - e.g. capture-upload Edge Function
-- The trigger enforce_ticket_basic_update_permissions blocked updates because has_capability(service_id, NULL, ...) fails.
...
BEGIN
  ...
  -- Allow service_role / backend operations (no authenticated user)
