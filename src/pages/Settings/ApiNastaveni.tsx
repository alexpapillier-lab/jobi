...
export function ApiNastaveni({ activeServiceId }: { activeServiceId: string | null }) {
  ...
  .then(data => {
    data.repairs.forEach(o => {
      ...
      <p style={popis}>
        Formulář na váš web má každý tarif – nastavíte ho v <strong style={{ color: "var(--text)" }}>Nastavení → Zakázky → Online rezervace</strong>.
        S veřejným API navíc nabídne model a opravu z vašeho ceníku i s předběžnou cenou a délkou opravy.
      </p>
