...
import { dnesDatum, type ZapujckaData } from "../../lib/zapujcka";
    ...
    return (
      <div style={{ display: "grid", gap: 8 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 200px), 1fr))", gap: 8 }}>
          <label style={stitek}>
            Zařízení
            <input type="text" value={navrh.nazev} onChange={(e) => setNavrh({ ...navrh, nazev: e.target.value })} placeholder="např. iPhone SE 2020, černý" style={input} autoFocus />
          </label>
          <label style={stitek}>
            Sériové číslo / IMEI
  ...
  if (!zapujcka) {
    return (
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <span style={{ fontSize: 12, color: "var(--muted)" }}>Zapíše se, co si zákazník odnáší, a vytiskne se smlouva o zápůjčce.</span>
      </div>
    );
  }
